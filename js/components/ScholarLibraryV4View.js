        ...
        <div class="ar5-frame">
            ...
            <main class="ar5-main">
                    ...
                    <section v-if="response && answer" class="ar5-results">
                        ...
                        <div v-if="sources.length" class="ar5-result-layout">
                            <div class="ar5-source-list">
                                <article v-for="source in sources" :key="source.citation_id" class="ar5-source-card" :class="{ active: selectedSourceId === source.citation_id }" :data-book-id="source.book_id || ''" :data-book-page="source.page || ''" @click="selectSource(source)">
                ...
                <template v-else>
                    ...
                    <section class="ar5-method-tech"><div><span>Moteur</span><strong>{{ engineLabel }} · hybride multilingue</strong></div><div><span>Corpus</span><strong>{{ formatNumber(status.books) }} ouvrages</strong></div><div><span>Index</span><strong>{{ status.fts_ready ? 'FTS actif' : 'À vérifier' }}</strong></div><div><span>Mode</span><strong>Preuves directes</strong></div></section>
                </template>
            </main>
        </div>
    </section>
