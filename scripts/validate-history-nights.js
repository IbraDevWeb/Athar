...
if (/new\s+Audio\s*\(|<audio|<iframe|fetch\s*\(/i.test(component)) fail('The launch reader must not depend on remote audio, iframes or fetch calls.');
...
if (!fakeToolView.components['root-tree-view']) fail('RootTreeView was not registered alongside History Nights.');
...
if (!fakeToolView.template.includes(`currentTool === 'roots'`)) fail('The roots route was not preserved.');
...
console.log(`History Nights validated: ${data.stories.length} stories, ${totalChapters} chapters, optional local narration and cache v31.`);
