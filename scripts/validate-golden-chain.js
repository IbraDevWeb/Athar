...
["const APP_VERSION = 'athar-pro-v33'","writeEarlyScript('isnad_data.js'","writeEarlyScript('js/components/GoldenChainView.js'",'css/golden-chain.css?v=${APP_VERSION}'].forEach(token => need(config, token, 'config.js'));
const worker = read('service-worker.js');
["const CACHE_VERSION = 'athar-pro-v33'",'./isnad_data.js?v=athar-pro-v33','./js/components/GoldenChainView.js?v=athar-pro-v33','./css/golden-chain.css?v=athar-pro-v33'].forEach(token => need(worker, token, 'service worker'));
need(read('extensions_data.js'),'Explorateur pédagogique des isnāds','metadata');
console.log(`Golden Chain validated: ${data.chains.length} chains, ${data.narrators.length} narrators and cache v33.`);
