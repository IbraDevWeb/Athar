...
try {
  await page.goto(`${base}/research-library.html`, { waitUntil: 'domcontentloaded' });
  await page.locator('html[data-athar-v40-continuity="athar-v40-continuity-1"]').waitFor({ state: 'attached', timeout: 15000 });
  const card = page.locator('[data-book-id="book-1"]');
  await card.waitFor({ state: 'visible', timeout: 15000 });
  await card.click();

  await page.locator('#readerView:not([hidden])').waitFor({ state: 'visible', timeout: 10000 });
  const passage = page.locator('.reader-passage').first();
  await passage.waitFor({ state: 'visible', timeout: 10000 });
  const ask = passage.locator('[data-ar402-research-passage]');
  await ask.waitFor({ state: 'visible', timeout: 10000 });
  assert.match((await ask.textContent()) || '', /Interroger ce passage/i);

  const recentsWhileReading = await page.evaluate(() => window.AtharV40Continuity?.readRecents?.() || []);
  assert.equal(recentsWhileReading.length, 1);
  assert.equal(recentsWhileReading[0].book_id, 'book-1');
  assert.equal(String(recentsWhileReading[0].page), '1');

  await page.locator('#closeReader').click();
  await page.locator('#libraryView:not([hidden])').waitFor({ state: 'visible', timeout: 10000 });
  const resume = page.locator('#ar402RecentReads [data-ar402-recent-book="book-1"]');
  await resume.waitFor({ state: 'visible', timeout: 10000 });
  assert.match((await resume.textContent()) || '', /Al-Muwaṭṭaʾ/);
  assert.match((await resume.textContent()) || '', /page 1/);

  await resume.click();
  await page.locator('#readerView:not([hidden])').waitFor({ state: 'visible', timeout: 10000 });
  const askAgain = page.locator('[data-ar402-research-passage]').first();
  await askAgain.waitFor({ state: 'visible', timeout: 10000 });
  await Promise.all([
  ...
  assert.match(draft, /Au nom d’Allah/);
  assert.deepEqual(errors, []);
  console.log('ATHAR V40.3 CONTINUITY JOURNEY: PASS');
} finally {
  await browser.close();
}
