...
try {
  ...
  await page.locator('.ar40-library-link').waitFor({ state: 'visible' });
  await page.locator('.ar40-composer-hint').waitFor({ state: 'visible' });
  assert.equal(await page.locator('html').getAttribute('data-athar-v40'), 'athar-v40-research-library-1');
  assert.ok(await page.locator('link[href*="athar-research-library-v40.css"]').count());

  await page.keyboard.press('/');
  assert.equal(await page.evaluate(() => document.activeElement?.matches('.ar5-composer textarea')), true);

  await page.locator('.ar5-rail nav button').filter({ hasText: 'Ouvrages' }).click();
  const researchBook = page.locator('.ar5-book-card[data-book-id="book-1"]');
  await researchBook.waitFor({ state: 'visible', timeout: 10000 });
  assert.equal(await researchBook.getAttribute('role'), 'button');
  assert.equal(await researchBook.getAttribute('tabindex'), '0');

  await Promise.all([
    page.waitForURL(url => url.pathname.endsWith('/research-library.html') && url.hash.includes('book=book-1'), { timeout: 15000 }),
    researchBook.click()
  ]);
  await page.locator('body.athar-v40-library').waitFor({ state: 'attached', timeout: 10000 });
  await page.locator('#readerView:not([hidden])').waitFor({ state: 'visible', timeout: 10000 });
  await page.locator('.ar40-reader-research').waitFor({ state: 'visible' });
  await page.locator('#readerPaper .reader-passage').waitFor({ state: 'visible' });
  ...
  console.log('ATHAR V40 RESEARCH + LIBRARY JOURNEY: PASS');
} finally {
  await browser.close();
}
