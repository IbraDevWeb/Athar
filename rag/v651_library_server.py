...
"""Athar V6.5.1 main server: V6.1 corpus + remote semantic sidecar fusion."""
...
from v651_remote_fusion import V651RemoteFusionRuntime, build_remote_runtime


class Handler(V5LibraryHandler):
    server_version = "AtharRAG/6.5.1-remote-semantic"

    def _runtime(self) -> V651RemoteFusionRuntime | None:
        runtime = getattr(self.server, "shard_runtime", None)
        return runtime if isinstance(runtime, V651RemoteFusionRuntime) else None
    ...
    def _status_payload(self) -> dict[str, Any]:
        # Keep status dynamic: remote availability/failures can change without a restart.
        if self.shard_runtime is not None:
            payload = dict(self.shard_runtime.status())
        else:
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Athar V6.5.1: corpus V6.1 avec fusion sémantique distante fail-open."
    )
    ...
    if getattr(server, "shard_runtime", None) is None:
        raise RuntimeError("V6.5.1 exige le corpus shardé.")
    runtime = build_remote_runtime(server.shard_runtime)
    # Never require the semantic service at startup. V6.1 remains immediately
    # available while the sidecar is cold, sleeping or temporarily unavailable.
    ...
    print(
        json.dumps(
            {
                "server": "athar-rag-v6.5.1-library",
